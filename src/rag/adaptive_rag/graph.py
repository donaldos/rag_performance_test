...
def build_adaptive_rag_graph(
    ...
    """
    Adaptive RAG 그래프를 구축하고 컴파일된 그래프를 반환한다.

    흐름:
        route_question
          → [general]      direct_generate → END
    ...
    """
    route_question      = make_route_question_node(llm_model)
    direct_generate     = make_direct_generate_node(llm_model)
    ...
    generate            = make_generate_node(llm_model, tag="Adaptive-RAG")
    check_hallucination = make_check_hallucination_node(llm_model)

    builder = StateGraph(GraphState)
    builder.add_node("route_question",      route_question)
    builder.add_node("direct_generate",     direct_generate)
