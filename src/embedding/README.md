...
python -m src.test.test_embedding --input src/test/output/파일_recursive_chunking.json

# 특정 모델 지정
python -m src.test.test_embedding --input 파일.json --embedder openai_small
python -m src.test.test_embedding --input 파일.json --embedder huggingface_ko

# 전체 모델 일괄 비교
python -m src.test.test_embedding --input 파일.json --all

# 벡터 미리보기
python -m src.test.test_embedding --input 파일.json --inspect --chunk-index 0

# 결과 JSON 저장 (→ VectorDB 단계 입력으로 사용)
python -m src.test.test_embedding --input 파일.json --embedder openai_small --save
```
...
```python
from src.embedding.strategies.huggingface_ko_strategy import HuggingFaceKoEmbeddingStrategy
from src.embedding.embedding_context import EmbeddingContext
