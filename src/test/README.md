...
python -m src.test.test_pdf_loading data/01.pdf --save

# 로더 직접 지정 + 저장
python -m src.test.test_pdf_loading data/01.pdf --loader pymupdf --save

# 저장 경로 직접 지정
python -m src.test.test_pdf_loading data/01.pdf --loader pdfplumber --save src/test/output/01_pdfplumber_loading.json

# 설치된 모든 로더 일괄 비교 (속도·품질 비교용)
python -m src.test.test_pdf_loading data/01.pdf --all

# 로딩 결과 내용 확인 (페이지별)
python -m src.test.test_pdf_loading data/01.pdf --inspect
python -m src.test.test_pdf_loading data/01.pdf --inspect --loader pdfplumber --page 2
...
python -m src.test.test_chunking --input src/test/output/01_pymupdf_loading.json

# 특정 전략 지정 + 저장
python -m src.test.test_chunking \
    --input src/test/output/01_pymupdf_loading.json \
    --chunker recursive \
    --save
...
python -m src.test.test_embedding \
    --input src/test/output/01_recursive_chunking.json
...
python -m src.test.test_vectordb \
    --input src/test/output/01_recursive_chunking_openai_small_embedding.json
...
# ① Loading
python -m src.test.test_pdf_loading $PDF --loader pymupdf --save
