...
사용법:
    ...
    python -m src.test.test_chunking --input src/test/output/file_pymupdf_loading.json

    # 특정 전략만 테스트
    python -m src.test.test_chunking --input docs.json --chunker recursive

    # 전체 전략 일괄 비교
    python -m src.test.test_chunking --input docs.json --all

    # 청킹 결과 전체 내용 확인
    python -m src.test.test_chunking --input docs.json --inspect
    python -m src.test.test_chunking --input docs.json --inspect --chunker semantic
    python -m src.test.test_chunking --input docs.json --inspect --chunk-index 0

    # 결과를 JSON으로 저장
    python -m src.test.test_chunking --input docs.json --chunker recursive --save
    python -m src.test.test_chunking --input docs.json --chunker recursive --save out/chunks.json
...
from src.chunking.chunking_context import ChunkingContext  # noqa: E402
from src.chunking.router import ChunkingRouter             # noqa: E402
from src.test.io_utils import (                            # noqa: E402
    default_save_path,
    load_documents,
    save_documents,
)
