...
사용법:
    ...
    python -m tests.test_embedding --input tests/output/file_recursive_chunking.json

    # 특정 임베더 지정
    python -m tests.test_embedding --input chunks.json --embedder openai_small
    python -m tests.test_embedding --input chunks.json --embedder huggingface_ko

    # 설치된 모든 임베더 일괄 비교
    python -m tests.test_embedding --input chunks.json --all

    # 임베딩 벡터 미리보기
    python -m tests.test_embedding --input chunks.json --inspect
    python -m tests.test_embedding --input chunks.json --inspect --chunk-index 2

    # 결과를 JSON으로 저장
    python -m tests.test_embedding --input chunks.json --embedder openai_small --save
    python -m tests.test_embedding --input chunks.json --embedder openai_small --save out/embedded.json
...
from ragsystem.embedding.router import EmbeddingRouter              # noqa: E402
from ragsystem.test.io_utils import default_save_path, load_documents  # noqa: E402
...
def _default_embed_save_path(input_path: str, embedder: str) -> Path:
    """저장 경로 자동 생성: tests/output/{stem}_{embedder}_embedding.json"""
