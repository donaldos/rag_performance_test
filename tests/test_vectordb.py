...
사용법:
    ...
    python -m tests.test_vectordb --input tests/output/file_openai_small_embedding.json

    # 특정 DB 지정
    python -m tests.test_vectordb --input embedded.json --db faiss
    python -m tests.test_vectordb --input embedded.json --db chromadb

    # 두 DB 모두 일괄 비교
    python -m tests.test_vectordb --input embedded.json --all

    # 구축 후 쿼리 검색
    python -m tests.test_vectordb --input embedded.json --db faiss --query "연구 방법론"
    python -m tests.test_vectordb --input embedded.json --db faiss --query "연구 방법론" --k 3

    # 인덱스를 디스크에 저장
    python -m tests.test_vectordb --input embedded.json --db faiss --save
    python -m tests.test_vectordb --input embedded.json --db chromadb --save out/chroma_index
...
from ragsystem.embedding.embedding_context import EmbeddingContext  # noqa: E402
from ragsystem.vectordb.vectordb_context import VectorDBContext     # noqa: E402
from ragsystem.vectordb.router import VectorDBRouter                # noqa: E402
from ragsystem.test.io_utils import load_embedded_chunks            # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help="인덱스를 디스크에 저장. PATH 생략 시 tests/output/ 자동 생성",
    )
